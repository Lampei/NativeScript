import * as definitionType from "./InspectorBackendCommands.ios";
var definition: typeof definitionType = require("./InspectorBackendCommands");
...
export class Request {
    ...
    public responseReceived(response: definitionType.NetworkDomain.Response): void {
        if (this._networkDomainDebugger.enabled) {
            this._networkDomainDebugger.events.responseReceived(this.requestID, "NativeScriptMainFrameIdentifier", "Loader Identifier", __inspectorTimestamp(), <any>this.resourceType, response);
        }
    }
    ...
    public requestWillBeSent(request: definitionType.NetworkDomain.Request): void {
        if (this._networkDomainDebugger.enabled) {
            this._networkDomainDebugger.events.requestWillBeSent(this.requestID, "NativeScriptMainFrameIdentifier", "Loader Identifier", request.url, request, __inspectorTimestamp(), { type: 'Script' });
        }
    }
}

@definition.DomainDispatcher("Network")
export class NetworkDomainDebugger implements definitionType.NetworkDomain.NetworkDomainDispatcher {
    private _enabled: boolean;
    public events: definitionType.NetworkDomain.NetworkFrontend;
     ...
     */
    setExtraHTTPHeaders(params: definitionType.NetworkDomain.SetExtraHTTPHeadersMethodArguments): void {
        //
    }
     ...
     */
    getResponseBody(params: definitionType.NetworkDomain.GetResponseBodyMethodArguments): { body: string, base64Encoded: boolean } {
     ...
     */
    setCacheDisabled(params: definitionType.NetworkDomain.SetCacheDisabledMethodArguments): void {
        //
    }
     ...
     */
    loadResource(params: definitionType.NetworkDomain.LoadResourceMethodArguments): { content: string, mimeType: string, status: number } {
        return {
            content: "",
            mimeType: "",
            status: 200
